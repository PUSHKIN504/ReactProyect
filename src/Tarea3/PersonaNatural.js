...
const [selectedCiudad, setSelectedCiudad] = useState(null
  ...
  useEffect(() => {
    const fetchEsCiv = async () => {
      try {
        const EsCiv = await getEsCiv();
        if (Array.isArray(EsCiv)) {
          setEsCiv(EsCiv);
        } else {
          throw new Error('Data format is incorrect');
        }
  ...
  const next = async () => {
    ...
    if(PersonaIngresada === '0'){
      setPersonaIngresada('1')
