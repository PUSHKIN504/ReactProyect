...
// const token = 'dff04c8383dabdc9539d14eacafd16b7';
// const prueba = https://api.themoviedb.org/3/list/1?api_key=dff04c8383dabdc9539d14eacafd16b7
export class UserList extends Component {
	...
	deleteRate = RateId => {
		...
		.then(data => {
			...
			this.setState({
				users: this.state.users.filter(item => item.id !== RateId),
			});
				...
				render: (_, elm) => (
					<div className="text-right d-flex justify-content-center">
						...
						<Tooltip title="Delete">
							<Button danger icon={<DeleteOutlined />} onClick={()=> {this.deleteRate(elm.id)}} size="small"/>
						</Tooltip>
					</div>
				)
			}
